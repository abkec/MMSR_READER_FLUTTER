...
class _ChangeChildGender_State extends State<ChangeChildGender> with SingleTickerProviderStateMixin {
  ...
  @override
  void initState() {
    if(widget.childData[0].children_gender ==  'M')//widget.childData[0] is the object from _ChangeChildrenGender_State
  ...
  Widget build(BuildContext context) {
    return new Scaffold(
      appBar: new AppBar(title: new Text('Update Gender',style:TextStyle(fontFamily: "WorkSansBold")),backgroundColor: Colors.lightBlue,
        actions: <Widget>[
          new Container(
  ...
  Widget _changeUsername(BuildContext context)// should rename to _updateChildGender
  {
    return Stack(
      children: <Widget>[
        Container(
          ...
          child: Row(
        children: <Widget>[
            Text('Gender',style: TextStyle(fontFamily: 'WorkSansSemiBold',fontSize: 18),),
            Radio(value: 1, groupValue: groupValue, onChanged:(e)=>radioOnchange(e),activeColor: Colors.blue,),
            Icon(FontAwesomeIcons.male),
            Text('Boy',style: TextStyle(fontFamily: 'WorkSansSemiBold',fontSize: 18)),
            Radio(value: 2, groupValue: groupValue, onChanged:(e)=>radioOnchange(e),activeColor: Colors.red),
            Icon(FontAwesomeIcons.female),
            Text('Girl',style: TextStyle(fontFamily: 'WorkSansSemiBold',fontSize: 18)),
      ],
    ),
        ),
      ],
  ...
  void updateGender()async
  {
    ...
    http.post(url+"updateChildren(Reader).php",body: {
      'children_name':widget.childData[0].children_name, //reuse all unchanged details
      'children_DOB':widget.childData[0].children_DOB,//reuse all unchanged details
      'children_gender':gender,
      'children_image':widget.childData[0].children_image,//reuse all unchanged details
      'children_id': widget.childData[0].children_id//reuse all unchanged details
    });
    String desc;
        if (widget.childData[0].children_gender == 'M')
          desc = widget.childData[0].children_id +
              ' has changed his gender';
        else
          desc = widget.childData[0].children_id +
    ...
    var db = DBHelper();
    var children = Children(widget.childData[0].children_id,loginID,widget.childData[0].children_name,widget.childData[0].children_DOB,gender,widget.childData[0].children_image);
