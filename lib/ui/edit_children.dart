  ...
  Widget build(BuildContext context) {
    ...
    return new Scaffold(
      ...
      body: new FutureBuilder<List>(
        future: db.getChildrenByID(widget.childData[widget.index].children_id),//start with "db.", data retrieve from local database. "widget.childData" is object of _LoadChildDataState
...
class _EditChildrenAccount_State extends State<EditChildrenAccount> with SingleTickerProviderStateMixin {
      ...
      child: Column(
        children: <Widget>[
          ListTile(
            ...
            onTap: (){
              Navigator.push(context,MaterialPageRoute(builder: (context)=> ChangeChildName(childData: widget.childData)),);
            },
            subtitle:  Text(widget.childData[0].children_name,style:TextStyle(fontSize: 18,fontFamily: "WorkSansMedium")),
            trailing: Icon(Icons.keyboard_arrow_right),
          ),
          ListTile(
            leading: Icon(FontAwesomeIcons.birthdayCake),
            title:Text('Birthdate',style: TextStyle(fontFamily: "WorkSansBold",fontSize: 20),),
            subtitle: Text(widget.childData[0].children_DOB,style:TextStyle(fontSize: 18,fontFamily: "WorkSansMedium")),
            onTap: (){
              Navigator.push(context,MaterialPageRoute(builder: (context)=> ChangeChildDOB(childData: widget.childData)),);

            },
            trailing: Icon(Icons.keyboard_arrow_right),
          ),
          ListTile(
            leading: widget.childData[0].children_gender == 'M' ?Icon(FontAwesomeIcons.male):Icon(FontAwesomeIcons.female),
            title:Text('Gender',style: TextStyle(fontFamily: "WorkSansBold",fontSize: 20),),
            subtitle: Text(widget.childData[0].children_gender,style:TextStyle(fontSize: 18,fontFamily: "WorkSansMedium")),
            onTap: (){
              Navigator.push(context,MaterialPageRoute(builder: (context)=> ChangeChildGender(childData: widget.childData)),);
          ...
          ListTile(
            ...
            title: Text("Change Profile Picture",style: TextStyle(fontFamily: "WorkSansBold",fontSize: 20),),
            onTap: (){
              showDialog(
                  context: context,
                  barrierDismissible: false,
                  builder: (_) => Center( // Aligns the container to center
                      child: SingleChildScrollView(// A simplified version of dialog.
                        child:  Container(
                          //height: 450,
...
class _ChangeProfilePicState extends State<ChangeProfilePic> {
  ...
  var pickImage =[];
  void createValue() //creating a list of "false". while user picked an image, then it will be "true"
  {
                ...
                child: Container(
                    ...
                    child: ListView.builder(
                        ...
                        itemBuilder: (context , i){
                          return Container(
                            color: pickImage[i]==false ? Colors.white:Colors.grey,//if value is true then become grey background
                            child: GestureDetector(
                              onTap: (){
                                setState(() {
                                  ...
                                  {//if the selected picture is false
                                    ...
                                    pickImage[i] = true;//then change it to true
                                    for(int j = 0 ; j!=i && j<animals.length; j++)
                                    {
                                   
                                      //make sure the rest of the value are false
                                      pickImage[j] = false;
                                    }
                                    for(int j = animals.length-1 ; j!=i && j>=0; j--)
                                    {
                                      //make sure the rest of the value are false
                                      pickImage[j] = false;
                                    }
                              ...
                              child:Card(
                                ...
                                child:Image.asset(animals[i],fit: BoxFit.cover,),
  ...
  void updateImage()async //Children change account picture
  {
    ...
    http.post(url+"updateChildren(Reader).php",body: {
      ...
      'children_gender':widget.childData[0].children_gender,//reuse unchanged data
    ...
    var db = DBHelper();
    var children = Children(widget.childData[0].children_id,loginID,widget.childData[0].children_name,widget.childData[0].children_DOB,widget.childData[0].children_gender,children_image);
